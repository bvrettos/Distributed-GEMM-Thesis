...
void Summa25Decomposer::communicationComputationOverlapInit()
{
    ...
    size_t totalExecutionMemoryRequirements = tileMemoryRequirements*(2*(pc3+1) + 1); // 1 Tile for C and 2*pc3 tiles for A, B
    ...
    if (freeMemory >= totalExecutionMemoryRequirements) {
        ...
        if (activeStreams > STREAM_HARD_CAP) activeStreams = STREAM_HARD_CAP;
    }

    else {
        /* Find out the maximum numbers of streams we can create and can utilize */
        activeStreams = freeMemory/totalExecutionMemoryRequirements;
        if (activeStreams < 1) {
            printf("Problem does not fit in GPUs, need to implement caching.\n");
    ...
    workspaceA = (double**) malloc(sizeof(double*) * activeStreams);
    workspaceB = (double**) malloc(sizeof(double*) * activeStreams);
    ...
    */
    ...
    // cudaMallocAsync((void**)&workspaceC, sizeof(double) * localM * localN, streams[0]);


    cudaMalloc((void**)&workspaceC, sizeof(double) * localM * localN);

    CUDA_CHECK(cudaStreamCreate(&cublasStream));
    CUDA_CHECK(cudaStreamCreate(&helperStream));
...
void Summa25Decomposer::multiplyNCCL(char TransA, char TransB, double* A, double* B, double* C, double alpha, double beta)
{
    ...
    */
    ...
    CUDA_CHECK(cudaEventRecord(*currentEvent, helperStream));
    /* Add a helper stream to free memory */
    // CUDA_CHECK(cudaFreeAsync((void*)&localA, helperStream));
    // CUDA_CHECK(cudaFreeAsync((void*)&localB, helperStream));
    ...
    CUBLAS_CHECK(cublasDgemm(cublasContext, charToCublasTransOp(TransA), charToCublasTransOp(TransB), 
        localM, localN, localK, &alpha, workspaceA[0], llda, workspaceB[0], lldb, &tempBeta, workspaceC, lldc));

    int nextAvailableStream = 0;
    if (activeStreams > 1) {
        nextAvailableStream++;
    }
    ...
    for (int rotationCount = 1; rotationCount < pc3; rotationCount++) {
        ...
        int computationIndex = rotationCount;
        int previousComputationIndex = computationIndex - 1;
        s = (processColumn + rotationCount) % pc;
        s_dot = (processRow + rotationCount) % pc;

        /* If activeStreams are same as pc3, then we are jet. */
        /* If not, need to use events on the oldest stream and synchronize with it. As soon as it is free, use it for the other task */
        ...
        /* Max overlap available, all next streams are available, use those and synchronize with events for computation. */
        if (activeStreams >= pc3) {
            currentCommunicationStream = &streams[rotationCount];
            currentEvent = &events[rotationCount];
        }

        else if (activeStreams < pc3) {
            /* Find out which stream could be free to use for communication */
            currentCommunicationStream = &streams[nextAvailableStream];
            currentEvent = &events[nextAvailableStream];
            computationIndex = nextAvailableStream;
            
            /* Check if previous event is complete */
            if (cudaEventQuery(*currentEvent) != cudaSuccess)
                cudaEventSynchronize(*currentEvent);

            nextAvailableStream++;
            if (nextAvailableStream > activeStreams) {
                nextAvailableStream = 0;
                previousComputationIndex = activeStreams-1;
            }
        }
        ...
        CUBLAS_CHECK(cublasDgemm(cublasContext, charToCublasTransOp(TransA), charToCublasTransOp(TransB), 
            localM, localN, localK, &alpha, workspaceA[computationIndex], llda, workspaceB[computationIndex], lldb, &tempBeta, workspaceC, lldc));
    }

    /* Synchronize device and reduce results*/
    cudaStreamSynchronize(cublasStream);
    reduceResultNCCL(workspaceC);
    ...
    int numberOfRuns, bool logging, bool gatherResults)
{
    ...
    if (processStack == 0) {
        ...
        MatrixInit(decomposer.workspaceC, localM * localN, 0);
    }
    ...
    for (int i = 0; i < numberOfRuns; i++) {
        ...
        if (gatherResults) {
            decomposer.reorderMatrix(0, C, decomposer.workspaceC);
        }
